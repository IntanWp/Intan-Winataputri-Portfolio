...
export function Navigation() {
  ...
  const navItems = [
    ...
    { name: "Credentials", id: "highlights" },
  ...
  return (
    <nav
      className={`fixed top-0 w-full z-50 transition-colors duration-300 ${
        showSolidNav
          ? "bg-background/95 backdrop-saturate-150 border-b border-border"
          : "bg-transparent"
      }`}
      style={showSolidNav ? { backdropFilter: "none" } : undefined}
    >
        ...
        <div className="flex justify-between items-center">
          ...
          <div className="hidden lg:flex items-center gap-8">
            ...
            <a
              ...
              download
              className="tag-chord text-[13px] label-wide px-4 py-2 rounded-md bg-accent text-accent-foreground hover:brightness-95 transition-all -rotate-1"
            >
              CV
            </a>
          </div>
        ...
        {isMobileMenuOpen && (
          <div className="lg:hidden mt-4 pb-2 border-t border-border pt-4 flex flex-col gap-1">
            ...
            <a
              href="/Intan-Winataputri-CV.pdf"
              download
              className="tag-chord text-sm label-wide text-left py-2.5 text-band-backend-ink"
            >
              Download CV ↓
            </a>
          </div>
