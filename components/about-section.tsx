...
export function AboutSection() {
  return (
    <section id="highlights" className="py-24 md:py-32 px-6 border-t border-border">
      <div className="max-w-6xl mx-auto">
        ...
        <dl className="max-w-3xl">
          {RECORD.map((row, i) => (
            ...
            >
              <dt className="tag-chord label-wide text-xs text-muted-foreground sm:w-40 shrink-0">{row.label}</dt>
              <dd className="font-sans text-lg md:text-xl text-foreground">
                {row.value}
                {row.note && <span className="tag-chord text-sm text-muted-foreground ml-3">— {row.note}</span>}
              </dd>
            </div>
          ))}
        </dl>
